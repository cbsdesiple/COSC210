...
public class GenericTree<T> implements Tree<T> {
    ...
    protected int size;
    protected int height;//keep track of the greatest dpth leaf here. soo that we dont have to calcuate it later
    ...
    protected TreeNode<T> searchHelper(T searchdata, TreeNode<T> node) {
        ...
        //we didn't find it lets check the children
        TreeNode<T>[] children = node.children();
    ...
    protected TreeNode<T> searchLevel(T searchdata, TreeNode<T>[] children) {
        ...
        if (children == null) {
            return null;// we made it to the end and didn't find it
        }
        for (int i = 0; i < children.length; i++) {
            TreeNode<T> child = children[i];
        ...
        //step 2 - recursivley search the cousins
        
        return searchLevel(searchdata, (TreeNode<T>[])cousins.toArray());
    }
}
